...
public class IntegerAggregatedExpressionToFeatureConverter extends ExpressionToFeatureConverter<IntegerAggregatedExpression> {

    private final JoinFactory joinFactory;

    public IntegerAggregatedExpressionToFeatureConverter(final FeatureFactory factory, final JoinFactory joinFactory, final AsmModelAdapter modelAdapter) {
        super(factory, modelAdapter);
        this.joinFactory = joinFactory;
    }

    @Override
    public Feature convert(final IntegerAggregatedExpression expression, final Context context, final FeatureTargetMapping targetMapping) {
        ...
        switch (expression.getOperator()) {
            case SUM: {
                signature = FunctionSignature.SUM_INTEGER;
                break;
            }
            case MIN: {
                signature = FunctionSignature.MIN_INTEGER;
                break;
            }
            case MAX: {
                signature = FunctionSignature.MAX_INTEGER;
                break;
            }
            default: {
