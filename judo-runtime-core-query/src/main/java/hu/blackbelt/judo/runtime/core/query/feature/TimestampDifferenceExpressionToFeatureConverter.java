...
public class TimestampDifferenceExpressionToFeatureConverter extends ExpressionToFeatureConverter<TimestampDifferenceExpression> {

    public TimestampDifferenceExpressionToFeatureConverter(FeatureFactory factory, AsmModelAdapter modelAdapter) {
        super(factory, modelAdapter);
    }

    @Override
    public Feature convert(final TimestampDifferenceExpression expression, final Context context, final FeatureTargetMapping targetMapping) {
        ...
        final Feature difference = newFunctionBuilder()
                .withSignature(FunctionSignature.DIFFERENCE_TIMESTAMP)
                .withParameters(newFunctionParameterBuilder()
                        .withParameterName(ParameterName.END)
                        .withParameterValue(factory.convert(expression.getEndTimestamp(), context, null))
                        .build())
                .withParameters(newFunctionParameterBuilder()
                        .withParameterName(ParameterName.START)
                        .withParameterValue(factory.convert(expression.getStartTimestamp(), context, null))
                        .build())
                .build();
