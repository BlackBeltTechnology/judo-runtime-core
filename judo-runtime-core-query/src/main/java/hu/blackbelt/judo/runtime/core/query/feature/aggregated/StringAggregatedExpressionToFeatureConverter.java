...
import hu.blackbelt.judo.meta.expression.string.StringAggregatedExpression;
...
public class StringAggregatedExpressionToFeatureConverter extends ExpressionToFeatureConverter<StringAggregatedExpression> {

    private final JoinFactory joinFactory;

    public StringAggregatedExpressionToFeatureConverter(final FeatureFactory factory, final JoinFactory joinFactory, final AsmModelAdapter modelAdapter) {
        super(factory, modelAdapter);
        this.joinFactory = joinFactory;
    }

    @Override
    public Feature convert(final StringAggregatedExpression expression, final Context context, final FeatureTargetMapping targetMapping) {
        ...
        switch (expression.getOperator()) {
            case MIN: {
                signature = FunctionSignature.MIN_STRING;
                break;
            }
            case MAX: {
                signature = FunctionSignature.MAX_STRING;
                break;
            }
            default: {
