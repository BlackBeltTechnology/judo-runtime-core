...
import hu.blackbelt.judo.meta.expression.temporal.TimeAggregatedExpression;
...
public class TimeAggregatedExpressionToFeatureConverter extends ExpressionToFeatureConverter<TimeAggregatedExpression> {

    private final JoinFactory joinFactory;

    public TimeAggregatedExpressionToFeatureConverter(final FeatureFactory factory, final JoinFactory joinFactory, final AsmModelAdapter modelAdapter) {
        super(factory, modelAdapter);
        this.joinFactory = joinFactory;
    }

    @Override
    public Feature convert(final TimeAggregatedExpression expression, final Context context, final FeatureTargetMapping targetMapping) {
        ...
        switch (expression.getOperator()) {
            case MIN: {
                signature = FunctionSignature.MIN_TIME;
                break;
            }
            case MAX: {
                signature = FunctionSignature.MAX_TIME;
                break;
            }
            case AVG: {
                signature = FunctionSignature.AVG_TIME;
                break;
            }
            default: {
