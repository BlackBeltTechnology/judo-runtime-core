...
import hu.blackbelt.judo.services.dao.core.processors.*;
...
import hu.blackbelt.judo.services.dao.rdbms.executors.ModifyStatementExecutor;
import hu.blackbelt.judo.services.dao.rdbms.executors.SelectStatementExecutor;
import hu.blackbelt.judo.services.dao.rdbms.executors.StatementExecutor;
import hu.blackbelt.judo.services.query.CustomJoinDefinition;
...
import static hu.blackbelt.judo.services.dao.rdbms.PayloadTraverser.traversePayload;
...
@Component(configurationPolicy = ConfigurationPolicy.REQUIRE,
        immediate = true, service = DAO.class)
@Designate(ocd = RdbmsDAOImpl.Config.class)
@NoArgsConstructor
...
public class RdbmsDAOImpl<ID> extends AbstractRdbmsDAO<ID> implements DAO<ID> {
    ...
    public @interface Config {
        ...
        @AttributeDefinition(name = "optimisticLockEnabled", description = "Optimistic lock enabled")
        boolean optimisticLockEnabled() default true;

        @AttributeDefinition(name = "chunkSize", description = "Chunk size to retrieve data from database")
    ...
    void start(Config config) {
        ...
        this.jooqEnabled = config.jooq_enabled();
        this.optimisticLockEnabled = config.optimisticLockEnabled();
        this.chunkSize = config.chunkSize();
        this.markSelectedRangeItems = config.markSelectedRangeItems();
    }
