...
import hu.blackbelt.judo.services.dao.rdbms.query.utils.RdbmsAliasUtil;
...
public class RdbmsResultSet extends RdbmsField {
    ...
    public RdbmsResultSet(@NonNull final SubSelect query, final boolean filterByInstances, final SubSelect parentIdFilterQuery, final RdbmsBuilder rdbmsBuilder, final DAO.Seek seek, final boolean withoutFeatures, final Map<String, Object> mask, final Map<String, Object> queryParameters, final boolean skipParents) {
        ...
        if (filterByInstances) {
            conditions.add(RdbmsFunction.builder()
                    ...
                    .parameters(Arrays.asList(
                            ...
                            RdbmsParameter.builder().parameterName(getInstanceIdsKey(query.getSelect())).build()
                    )).build());
        }
        ...
        if (!withoutFeatures || query.getSelect().isAggregated()) {
            joins.addAll(subSelects.stream()
                    ...
                    .filter(s -> query.getSelect().getFeatures().stream().anyMatch(f -> f.getNodes().stream().anyMatch(n -> AsmUtils.equals(n, s.getSelect()) || s.getSelect().getJoins().contains(n))))
                    .map(a -> RdbmsQueryJoin.builder()
                            .resultSet(new RdbmsResultSet(a, false, parentIdFilterQuery, rdbmsBuilder, null, withoutFeatures, null, queryParameters, false))
                            .outer(true)
                            .columnName(getOptionalParentIdColumnAlias(a.getContainer()))
                            .partnerTable(a.getNavigationJoins().isEmpty() ? null : a.getContainer())
                            .partnerColumnName(a.getNavigationJoins().isEmpty() ? null : StatementExecutor.ID_COLUMN_NAME)
                            .alias(a.getAlias())
                            .build())
                    .collect(Collectors.toList()));
        }
        ...
        if (!query.getNavigationJoins().isEmpty()) {
            ...
            final RdbmsNavigationJoin customJoin = new RdbmsNavigationJoin(query, parentIdFilterQuery, rdbmsBuilder, withoutFeatures, queryParameters);
    ...
    private void addFilterJoinsAndConditions(final Filter filter, final RdbmsBuilder rdbmsBuilder, final SubSelect parentIdFilterQuery, final Node partnerTable, final String partnerTablePrefix, final boolean addJoinsOfFilterFeature, final Map<String, Object> queryParameters) {
        ...
        joins.addAll(filter.getFeatures().stream()
                ...
                .map(f -> RdbmsQueryJoin.builder()
                        .resultSet(new RdbmsResultSet(f.getSubSelect(), false, parentIdFilterQuery, rdbmsBuilder, null,true, null, queryParameters, false))
                        .outer(true)
                        .columnName(getOptionalParentIdColumnAlias(f.getSubSelect().getContainer()))
    ...
    public String toSql(final String prefix, final boolean includeAlias, final Coercer coercer, final MapSqlParameterSource sqlParameters, final EMap<Node, String> prefixes) {
        ...
        final String dual = dialect.getDualTable();
        final String sql = //"-- " + newPrefixes.stream().map(p -> p.getKey().getAlias() + ": " + p.getValue()).collect(Collectors.joining(", ")) + "\n" +
                "SELECT " + (addDistinct ? "DISTINCT " : "") + columns.stream().map(c -> c.toSql(prefix, true, coercer, sqlParameters, newPrefixes)).collect(Collectors.joining(", ")) +
                        ...
                        (group ? "\nGROUP BY " + prefix + RdbmsAliasUtil.AGGREGATE_PREFIX + query.getAlias() + "." + getParentIdColumnAlias(query.getContainer()) : "") +
