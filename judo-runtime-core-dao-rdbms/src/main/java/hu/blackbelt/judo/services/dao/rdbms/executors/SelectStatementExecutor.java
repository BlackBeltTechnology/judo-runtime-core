...
import hu.blackbelt.judo.services.dao.rdbms.query.AncestorNameFactory;
...
import hu.blackbelt.judo.services.dao.rdbms.query.model.RdbmsResultSet;
import hu.blackbelt.judo.services.dao.rdbms.query.translators.*;
...
import static hu.blackbelt.judo.services.dao.rdbms.query.utils.RdbmsAliasUtil.*;
...
public class SelectStatementExecutor<ID> extends StatementExecutor<ID> {
    ...
    public Collection<Payload> executeSelect(final NamedParameterJdbcTemplate jdbcTemplate, final EReference reference, final Collection<ID> ids, final DAO.QueryCustomizer<ID> queryCustomizer) {
        try (MetricsCancelToken ct = metricsCollector.start(METRICS_SELECT_PREPARE)) {
            ...
            final String subParentKey = getParentIdColumnAlias(query.getContainer());
    ...
    private EMap<Target, Map<ID, Payload>> runQuery(final NamedParameterJdbcTemplate jdbcTemplate, final SubSelect query, final Collection<ID> instanceIds, final Collection<ID> parentIds, final EList<EReference> referenceChain, final DAO.Seek seek, final boolean withoutFeatures, final Map<String, Object> mask, final Map<String, Object> queryParameters, final boolean skipParents) {
        ...
        final Map<String, Node> sources = new ConcurrentHashMap<>(query.getSelect().getAllJoins().stream().collect(Collectors.toMap(j -> getIdColumnAlias(j), j -> j)));
        sources.put(getIdColumnAlias(query.getSelect()), query.getSelect());

        final Map<String, Node> types = new ConcurrentHashMap<>(query.getSelect().getAllJoins().stream().collect(Collectors.toMap(j -> getTypeColumnAlias(j), j -> j)));
        types.put(getTypeColumnAlias(query.getSelect()), query.getSelect());

        final Map<String, Node> versions = new ConcurrentHashMap<>(query.getSelect().getAllJoins().stream().collect(Collectors.toMap(j -> getVersionColumnAlias(j), j -> j)));
        versions.put(getVersionColumnAlias(query.getSelect()), query.getSelect());

        final Map<String, Node> createUsernames = new ConcurrentHashMap<>(query.getSelect().getAllJoins().stream().collect(Collectors.toMap(j -> getCreateUsernameColumnAlias(j), j -> j)));
        createUsernames.put(getCreateUsernameColumnAlias(query.getSelect()), query.getSelect());

        final Map<String, Node> createUserIds = new ConcurrentHashMap<>(query.getSelect().getAllJoins().stream().collect(Collectors.toMap(j -> getCreateUserIdColumnAlias(j), j -> j)));
        createUserIds.put(getCreateUserIdColumnAlias(query.getSelect()), query.getSelect());

        final Map<String, Node> createTimestamps = new ConcurrentHashMap<>(query.getSelect().getAllJoins().stream().collect(Collectors.toMap(j -> getCreateTimestampColumnAlias(j), j -> j)));
        createTimestamps.put(getCreateTimestampColumnAlias(query.getSelect()), query.getSelect());

        final Map<String, Node> updateUsernames = new ConcurrentHashMap<>(query.getSelect().getAllJoins().stream().collect(Collectors.toMap(j -> getUpdateUsernameColumnAlias(j), j -> j)));
        updateUsernames.put(getUpdateUsernameColumnAlias(query.getSelect()), query.getSelect());

        final Map<String, Node> updateUserIds = new ConcurrentHashMap<>(query.getSelect().getAllJoins().stream().collect(Collectors.toMap(j -> getUpdateUserIdColumnAlias(j), j -> j)));
        updateUserIds.put(getUpdateUserIdColumnAlias(query.getSelect()), query.getSelect());

        final Map<String, Node> updateTimestamps = new ConcurrentHashMap<>(query.getSelect().getAllJoins().stream().collect(Collectors.toMap(j -> getUpdateTimestampColumnAlias(j), j -> j)));
        updateTimestamps.put(getUpdateTimestampColumnAlias(query.getSelect()), query.getSelect());
        ...
        chunks.forEach(chunk -> {
            ...
            if (chunk.parentIds != null) {
                sqlParameters.addValue(getParentIdsKey(query.getSelect()), chunk.parentIds.stream().map(id -> coercer.coerce(id, parameterMapper.getIdClassName())).collect(Collectors.toList()));
            }
            if (chunk.instanceIds != null) {
                sqlParameters.addValue(getInstanceIdsKey(query.getSelect()), chunk.instanceIds.stream().map(id -> coercer.coerce(id, parameterMapper.getIdClassName())).collect(Collectors.toList()));
            }

            // key used to identify parent instance in subselects
            final String parentKey = getParentIdColumnAlias(query.getContainer());
                            ...
                            query.getSelect().getTargets().stream().forEach(target -> {
                                ...
                                final Optional<FeatureTargetMapping> featureTargetMapping = query.getSelect().getFeatures().stream()
                                        .flatMap(f -> f.getTargetMappings().stream()
                                                .filter(tm -> AsmUtils.equals(tm.getTarget(), target) && tm.getTargetAttribute() != null && getTargetColumnAlias(target, RdbmsMapper.getAttributeOrFeatureName(tm.getTargetAttribute(), null)).equalsIgnoreCase(field.getKey())))
                                        .findAny();
    ...
    private void executeSubQuery(final NamedParameterJdbcTemplate jdbcTemplate, final SubSelect query, final SubSelect subSelect, final EList<EReference> newReferenceChain, final EMap<Target, Map<ID, Payload>> results, final Collection<ID> ids, final Map<String, Object> mask, final Map<String, Object> queryParameters) {
        ...
        final String subParentKey = getParentIdColumnAlias(subSelect.getContainer());
