...
public class RdbmsNavigationJoin extends RdbmsJoin {
    ...
    public RdbmsNavigationJoin(final SubSelect query, final SubSelect parentIdFilterQuery, final RdbmsBuilder rdbmsBuilder, final boolean withoutFeatures, final Map<String, Object> queryParameters) {
        ...
        if (query.getPartner() != null) {
            columnName = getIdColumnAlias(query.getPartner());
        ...
        subJoins.addAll(aggregations.stream()
                .map(subSelect -> RdbmsQueryJoin.builder()
                        ...
                        .outer(true)
                        .columnName(getOptionalParentIdColumnAlias(subSelect.getContainer()))
        ...
        if (query.getPartner() != null) {
            subFeatures.add(RdbmsColumn.builder()
                    ...
                    .columnName(StatementExecutor.ID_COLUMN_NAME)
                    .alias(getIdColumnAlias(query.getPartner()))
                    .build());
        ...
        if (parentIdFilterQuery != null && AsmUtils.equals(parentIdFilterQuery.getContainer(), query.getBase())) {
            subConditions.add(RdbmsFunction.builder()
                    .pattern("{0} IN (:" + getParentIdsKey(parentIdFilterQuery.getSelect()) + ")")
                    .parameter(RdbmsColumn.builder()
                            .partnerTable(query.getBase())
                            .columnName(StatementExecutor.ID_COLUMN_NAME)
                            .build())
    ...
    protected String getTableNameOrSubQuery(final String prefix, final Coercer coercer, final MapSqlParameterSource sqlParameters, final EMap<Node, String> prefixes) {
        final String subSelectJoinPrefix = prefix + getNavigationSubSelectAlias(query);
        ...
        final String sql = //"-- " + newPrefixes.stream().map(p -> p.getKey().getAlias() + ": " + p.getValue()).collect(Collectors.joining(", ")) + "\n" +
                ...
                subJoins.stream().map(j -> "    " + j.toSql(subSelectJoinPrefix, coercer, sqlParameters, newPrefixes, false)).collect(Collectors.joining()) +
