...
class JarSkillPackageTest {
    ...
    void marketplaceJsonShouldHaveVersionSubstituted() {
        ...
        assertThat("marketplace.json should contain module name",
            content, containsString("judo-runtime-core-dao-rdbms"));
    }

    @Test
    void pluginJsonShouldBeAccessibleViaClasspath() {
        InputStream is = getClass().getResourceAsStream(
            "/claude/plugins/judo-dao-rdbms/.claude-plugin/plugin.json");
        assertThat("plugin.json should be accessible via classpath", is, notNullValue());
    }

    @Test
    void customQueriesSkillShouldBeAccessible() {
        InputStream is = getClass().getResourceAsStream(
            "/claude/plugins/judo-dao-rdbms/skills/custom-queries/SKILL.md");
        assertThat("custom-queries skill should be accessible", is, notNullValue());
    }

    @Test
    void queryDebuggingSkillShouldBeAccessible() {
        InputStream is = getClass().getResourceAsStream(
            "/claude/plugins/judo-dao-rdbms/skills/query-debugging/SKILL.md");
        assertThat("query-debugging skill should be accessible", is, notNullValue());
    }

    @Test
    void dialectExtensionSkillShouldBeAccessible() {
        InputStream is = getClass().getResourceAsStream(
            "/claude/plugins/judo-dao-rdbms/skills/dialect-extension/SKILL.md");
        assertThat("dialect-extension skill should be accessible", is, notNullValue());
    }
